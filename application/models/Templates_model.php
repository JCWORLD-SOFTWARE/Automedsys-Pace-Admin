...
class Templates_model extends Base_model {
    ...
    public function ComboBoxData($name,$value,$data){
        return $this->combo_box_data($data,$name,$value,'ID',['server_name','template_name','template_description']);
    }
    ...
    public function load_templates() {
        ...
        try {
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                ...
                'SOAPAction: "http://automedsys.net/webservices/PracticeDatabaseServerTemplateList"'
            ));
            curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
            curl_setopt( $ch, CURLOPT_POSTFIELDS, "<?xml version=\"1.0\" encoding=\"utf-8\"?>
            <soap:Envelope xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">
              <soap:Body>
                <PracticeDatabaseServerTemplateList xmlns=\"http://automedsys.net/webservices\">
                  <limit>${limit}</limit>
                  <offset>${offset}</offset>
                  <sessionid>".$_SESSION["session"]."</sessionid>
                </PracticeDatabaseServerTemplateList>
              </soap:Body>
            </soap:Envelope>");
            ...
            if (is_object($xml) && property_exists($xml,'Body') 
             && is_object($xml->Body) && property_exists($xml->Body,'PracticeDatabaseServerTemplateListResponse') 
             && is_object($xml->Body->PracticeDatabaseServerTemplateListResponse) 
             && property_exists($xml->Body->PracticeDatabaseServerTemplateListResponse,'PracticeDatabaseServerTemplateListResult')
             && is_object($xml->Body->PracticeDatabaseServerTemplateListResponse->PracticeDatabaseServerTemplateListResult)) {
                // Process response 
                $res = $xml->Body->PracticeDatabaseServerTemplateListResponse->PracticeDatabaseServerTemplateListResult;
                //var_dump($res);
