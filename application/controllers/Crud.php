...
class Crud extends CI_Controller {
    ...
    public function features() {
        ...
        $this->load->model('Features_model'); // call model
        $res = $this->Features_model->retreive();
        $data['res'] = $res;

        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_features', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }
    ...
    public function featurecreate() {
        ...
        if ($_POST) {
            $in = array();
            $in['feature'] = trim($this->input->post('feature'));
            $in['list_order'] = (int)$this->input->post('list_order');
            $in['short_detail'] = trim($this->input->post('short_detail'));
            $in['long_detail'] = trim($this->input->post('long_detail'));
            $in['feature_group'] = (int)$this->input->post('feature_group');
            $in['status'] = (int)$this->input->post('status'); 
            
            if ($in['feature']!='' && $in['short_detail']!='' && $in['long_detail']!='') {
                $id = $this->Features_model->create($in);
                if ($id>0) {
                    redirect(base_url().'/crud/featureupdate?id='.$id);
                    return;
                } else {
                    $data['message'] = 'Failed to create record';
                }
        ...
        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_features_create', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }
    ...
    public function featureupdate() {
        ...
        if ($_POST) {
            ...
            if ($id>0 && $in['feature']!='' && $in['short_detail']!='' && $in['long_detail']!='') {
                $id = $this->Features_model->update($in);
                if ($id>0) {
                    $data['message'] = 'Record updated';
                } else {
                    $data['message'] = 'Failed to update record';
        ...
        $res = $this->Features_model->retreive($id);
        if (is_array($res) && count($res)>0) {
            $item = $res[0];
            $data['feature'] = $item['feature'];
            $data['list_order'] = $item['list_order'];
            $data['short_detail'] = $item['short_detail'];
            $data['long_detail'] = $item['long_detail'];
            $data['feature_group'] = $item['feature_group'];
            $data['status'] = $item['status'];
        } 

        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_features_update', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }
    ...
    public function featuredelete() {
        ...
        redirect(base_url().'/crud/features');
    }

    public function bkousers() {
        $this->checkSession();
        $data = array();

        $this->load->model('Bkousers_model'); // call model
        $res = $this->Bkousers_model->retreive();
        $data['res'] = $res;

        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_bkousers', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }

    public function bkousercreate() {
        $this->checkSession();
        $data = array();
        $data['username'] = '';
        $data['name'] = '';
        ...
        if ($_POST) {
            ...
            $in['name'] = trim($this->input->post('name'));
            $in['pass'] = trim($this->input->post('pass'));
            $in['status'] = $this->input->post('status'); 
            
            if ($in['username']!='' && $in['name']!='' && $in['pass']!='') {
                $in['pass'] = md5($in['pass']);
                $id = $this->Bkousers_model->create($in);
                if ($id>0) {
                    redirect(base_url().'crud/bkouserupdate?id='.$id);
                    return;
                } else {
                    $data['message'] = 'Failed to create record';
                }
        ...
        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_bkousers_create', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }

    public function bkouserupdate() {
        $this->checkSession();
        $data = array();
        $data['username'] = '';
        $data['name'] = '';
        ...
        if ($_POST) {
            ...
            if ($id>0 && $in['username']!='' && $in['name']!='') {
                if ($in['pass']!='') {
                    $in['pass'] = md5($in['pass']);
                } else {
                    unset($in['pass']);
                }
                $id = $this->Bkousers_model->update($in);
                if ($id>0) {
                    $data['message'] = 'Record updated';
                } else {
                    $data['message'] = 'Failed to update record';
        ...
        $res = $this->Bkousers_model->retreive($id);
        if (is_array($res) && count($res)>0) {
            $item = $res[0];
        ...
        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_bkousers_update', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }

    public function bkouserdelete() {
        $this->checkSession();
        $id = $this->input->get('id');
        if ($id>0) {
            $this->load->model('Bkousers_model'); // call model
            $this->Bkousers_model->delete($id);
        }
        redirect(base_url().'/crud/bkousers');
    }
    ...
    public function practiceaccounts() {
        $this->checkSession();
        $data = array();

        $this->load->model('Practiceaccounts_model'); // call model
        $res = $this->Practiceaccounts_model->retreive();
        $data['res'] = $res;

        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_practiceaccounts', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }

    public function practiceaccountcreate() {
        $this->checkSession();
        $data = array();
        $data['username'] = '';
        $data['online_username'] = '';
        ...
        if ($_POST) {
            ...
            if ($in['username']!='' && $in['online_username']!='' && $in['practice_code']!='') {
                $id = $this->Practiceaccounts_model->create($in);
                if ($id>0) {
                    redirect(base_url().'/crud/practiceaccountupdate?id='.$id);
                    return;
                } else {
                    $data['message'] = 'Failed to create record';
                }
        ...
        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_practiceaccounts_create', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }

    public function practiceaccountupdate() {
        $this->checkSession();
        $data = array();
        $data['username'] = '';
        $data['online_username'] = '';
        ...
        if ($_POST) {
            ...
            if ($id>0 && $in['username']!='' && $in['online_username']!='' && $in['practice_code']!='') {
                $id = $this->Practiceaccounts_model->update($in);
                if ($id>0) {
                    $data['message'] = 'Record updated';
                } else {
                    $data['message'] = 'Failed to update record';
        ...
        $res = $this->Practiceaccounts_model->retreive($id);
        if (is_array($res) && count($res)>0) {
            $item = $res[0];
            $data['username'] = $item['username'];
            $data['online_username'] = $item['online_username'];
        ...
        $this->load->view('tmpl/header_authsecure', $data);
        $this->load->view('crud/view_practiceaccounts_update', $data);
        $this->load->view('tmpl/footer_authsecure', $data);
    }

    public function practiceaccountdelete() {
        $this->checkSession();
        $id = $this->input->get('id');
        if ($id>0) {
            $this->load->model('Practiceaccounts_model'); // call model
            $this->Practiceaccounts_model->delete($id);
        }
        redirect(base_url().'/crud/practiceaccounts');
    }
